    ...
    <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-slate-900/90 to-purple-900/50 p-8 backdrop-blur-sm">
      ...
      <div className="mb-6 flex items-start justify-between">
        <div className="flex-1">
          ...
          {metadata.soundDescription && (
            <p className="mt-2 italic text-purple-300">🎵 {metadata.soundDescription}</p>
          )}
        </div>
      </div>
      ...
      <div className="mb-8 flex flex-wrap gap-2">
        ...
        {metadata.genres.map((genre) => (
          <span
            key={genre}
            className="rounded-lg bg-pink-500/20 px-3 py-1 text-sm text-pink-200"
          >
            {genre}
          </span>
        ))}
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-semibold text-pink-300">
                      {connection.to.moduleName}
                    </span>
                    <span className="text-slate-400">•</span>
                    <span className="text-slate-300">{connection.to.inputName}</span>
                  </div>
                  {connection.note && (
                    <p className="mt-1 text-xs italic text-slate-400">{connection.note}</p>
                  )}
                </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {parameterSuggestions.map((param, index) => (
              ...
              >
                ...
                {param.reasoning && (
                  <p className="mt-2 text-xs italic text-slate-400">{param.reasoning}</p>
                )}
              </div>
            ))}
          </div>
      ...
      <div className="flex gap-3">
        <button className="flex-1 rounded-xl bg-purple-500 px-6 py-3 font-semibold text-white transition-all hover:bg-purple-600 hover:scale-105">
          Save to Cookbook
        </button>
